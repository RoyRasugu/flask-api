from .db_model import Heroku_Db
import jwt
...
class UserModels():
    ...
    def create_user(self, username, email, userpassword, confirm_password):
        ...
        duplicate_email = Heroku_Db.retrieve_all(email_query)
        if duplicate_email:
            return False
        ...
        response = Heroku_Db.add_to_db(user_query, user_data)
        return response

    def get_user_by_email(self, email):
        """Get user by email"""
        user_email_query = f"SELECT * FROM users WHERE email = {email}"
        user_response = Heroku_Db.retrieve_one(user_email_query)
    ...
    def get_user_by_id(self, id):
        ...
        user_id_query = """SELECT * FROM users WHERE userId = '{}'""".format(
            id)
        user_response = Heroku_Db.retrieve_one(user_id_query)
        if not user_response:
            return False
        return user_response
    ...
    def validate_password(self, userpassword, user_email):
        ...
        query = """SELECT userpassword FROM users where email='{}'""".format(
            user_email)
        result = Heroku_Db.retrieve_one(query)
